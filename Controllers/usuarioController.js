...
const registrarParticipante = async (req, res) => {
  ...
  try {
    ...
    const participanteR = pool.query(queryRegistro, [
      participante.nombre,
...
const obtenerParticipante = async (req, res) => {
  const { id } = req.params;
  try {
    const queryParticipante = `SELECT * FROM participante where id_participante = ?`;

    const participante = await pool.query(queryParticipante, [id]);

    if (participante[0].length < 1) {
...
const obtenerParticipantePorNombre = async (req, res) => {
  ...
  try {
    const queryParticipante = `SELECT * FROM participante where nombre like ?`;
